...
int main( int argc, char *argv[] ) {
    ...
    for ( unsigned int n = 1; n <= numberOfRounds; n ++ ) {
        ...
        // Assemble xi in double and build sparse CSR (float values) from edges
        double *xi = (double*) calloc( dim, sizeof( double ) );

        CSRMatrix csr;
        build_csr_from_edges_f( edges, size_of_edges, dim, &csr, xi );

        // Cast xi to float for the sparse PCG solver
        float *xi_f = (float*) malloc( sizeof( float ) * dim );
        ...
        pcg_solve_sparse( delta_xs_f, &csr, xi_f, dim );

        free( xi_f );
        free_csr( &csr );

        // Cast float result back to double for pose update
        double *delta_xs = (double*) malloc( sizeof( double ) * dim );
        for ( unsigned int i = 0; i < dim; i ++ ) {
            delta_xs[ i ] = (double)delta_xs_f[ i ];
        }
