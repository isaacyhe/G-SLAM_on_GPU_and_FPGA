...
void build_csr_from_edges_f( struct Edge *edges, unsigned int n_edges,
    ...
    // Use a dense boolean scratch array (dim x dim bits would be large; use
    // a dim x dim byte array — acceptable for the problem sizes here).
    // -----------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// pcg_solve_sparse — block-Jacobi preconditioned CG, all float, sparse SpMV.
//
// Preconditioner: invert each 3x3 diagonal block analytically.
// The diagonal block for block blk is extracted from the CSR by looking up
// the 9 entries (base+r, base+c) for r,c in [0,3).
// ---------------------------------------------------------------------------
void pcg_solve_sparse( float *x, CSRMatrix *csr, float *b, unsigned int dim ) {
    unsigned int n_blocks = dim / 3;

    // Build block-diagonal inverse (stored as n_blocks * 9 floats)
    float *diag_inv = (float*)calloc( n_blocks * 9, sizeof(float) );
    ...
    for ( unsigned int blk = 0; blk < n_blocks; blk++ ) {
        ...
        // Invert 3x3 block analytically via cofactor formula
    ...
    // Allocate PCG working vectors
    float *r_vec  = (float*)calloc( dim, sizeof(float) );
    float *z      = (float*)calloc( dim, sizeof(float) );
    float *p_vec  = (float*)calloc( dim, sizeof(float) );
    float *q      = (float*)calloc( dim, sizeof(float) );

    // Initialize x = 0
    for ( unsigned int i = 0; i < dim; i++ ) x[i] = 0.0f;

    // r = b - A*x = b (since x = 0)
    for ( unsigned int i = 0; i < dim; i++ ) r_vec[i] = b[i];

    // z = M_inv * r  (block-Jacobi apply)
    for ( unsigned int blk = 0; blk < n_blocks; blk++ ) {
        unsigned int base = blk * 3;
        for ( unsigned int r = 0; r < 3; r++ ) {
            z[base+r] = 0.0f;
            for ( unsigned int c = 0; c < 3; c++ )
                z[base+r] += diag_inv[blk*9 + r*3+c] * r_vec[base+c];
        }
    }
    ...
    for ( unsigned int k = 0; k < max_iter; k++ ) {
        ...
        float r_norm = 0.0f;
        for ( unsigned int i = 0; i < dim; i++ ) r_norm += r_vec[i] * r_vec[i];
        r_norm = sqrtf( r_norm );
        if ( r_norm < tol ) break;
