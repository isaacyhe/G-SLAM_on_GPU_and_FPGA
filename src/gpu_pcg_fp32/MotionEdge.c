...
struct Edge* MotionEdge_create ( unsigned int t1, unsigned int t2, struct HAT_X *hat_xs, struct U *us, unsigned int delta, float lambda, float *mns ) { // The constructor
    ...
    float M[ 2 * 2 ] = {
        mns[ 0 ] * mns[ 0 ] * fabsf( nu ) / delta + mns[ 1 ] * mns[ 1 ] * fabsf( omega ) / delta, 0.0f,
        0.0f, mns[ 2 ] * mns[ 2 ] * fabsf( nu ) / delta + mns[ 3 ] * mns[ 3 ] * fabsf( omega ) / delta
    };
    ...
    float st = sinf( motion_edge_self->hat_x1.hat_x[2] );
    float ct = cosf( motion_edge_self->hat_x1.hat_x[2] );

    float stw = sinf( motion_edge_self->hat_x1.hat_x[2] + omega * delta );
    float ctw = cosf( motion_edge_self->hat_x1.hat_x[2] + omega * delta );
    ...
    F[ 3 * 0 + 2 ] = nu / omega * cosf( motion_edge_self->hat_x1.hat_x[2] + omega * delta ) - nu / omega * cosf( motion_edge_self->hat_x1.hat_x[2] );
    F[ 3 * 1 + 2 ] = nu / omega * sinf( motion_edge_self->hat_x1.hat_x[2] + omega * delta ) - nu / omega * sinf( motion_edge_self->hat_x1.hat_x[2] );
    ...
    float transposeOfA[ 2 * 3 ] = { 0.0f };
    tra( transposeOfA, A, 3, 2 );

    float dot_productOfAM[ 3 * 2 ] = { 0.0f };
    dot( dot_productOfAM, A, 3, 2, M, 2, 2 );

    float dot_productOfDPtA[ 3 * 3 ] = { 0.0f };
    dot( dot_productOfDPtA, dot_productOfAM, 3, 2, transposeOfA, 2, 3 );

    float multiplicationOfN[ 3 * 3 ] = { 0.0f };
    mul( multiplicationOfN, NP_EYE, 3, 3, 0.0001f );

    float addition[ 3 * 3 ] = { 0.0f };
    ...
    float multiplicationOfDPL_2[ 3 * 3 ] = { 0.0f };
    ...
    float dot_productOftmOF[ 3 * 3 ] = { 0.0f };
    dot( dot_productOftmOF, motion_edge_self->Omega, 3, 3, F, 3, 3 );

    float multiplicationOfDPL_3[ 3 * 3 ] = { 0.0f };
    ...
    float multiplicationOfmOL[ 3 * 3 ] = { 0.0f };
    ...
    float dot_productOfmOS[ 3 * 1 ] = { 0.0f };
    ...
    float multiplicationOfDPL_5[ 3 * 1 ] = { 0.0f };
    ...
    for( unsigned int i = 0; i < 2 * 1; i ++ ) {
		motion_edge_self->hat_e[ i ] = 0.0f; // Not needed by MotionEdge
    }
