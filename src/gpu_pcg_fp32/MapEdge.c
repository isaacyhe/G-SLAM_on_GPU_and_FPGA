...
struct MapEdge* MapEdge_create ( struct Z z, struct Z head_z, struct HAT_X *hat_xs, float *snr ) { // The constructor
    ...
    float m2[ 2 * 1 ] = {
		z.z[0] * cosf( map_edge_self->x.hat_x[2] + z.z[1] ),
		z.z[0] * sinf( map_edge_self->x.hat_x[2] + z.z[1] )
    };

    float additionOfMM[ 2 * 1 ] = { 0.0f };
    ...
    float Q1[ 2 * 2 ] = {
	    map_edge_self->z.z[0] * powf( snr[0], 2 ) , 0.0f,
	    0.0f, powf( snr[1], 2 )
    };

    float s1 = sinf( map_edge_self->x.hat_x[2] + map_edge_self->z.z[1] );
    float c1 = cosf( map_edge_self->x.hat_x[2] + map_edge_self->z.z[1] );
    ...
    float dot_productOfRQ[ 2 * 2 ] = { 0.0f };
    dot( dot_productOfRQ, R, 2, 2, Q1, 2, 2 );

    float transposeOfR[ 2 * 2 ] = { 0.0f };
    tra(  transposeOfR, R, 2, 2 );

    float dot_productOfDPtR[ 2 * 2 ] = { 0.0f };
    ...
    float dot_productOfmOmm[ 2 * 1 ] = { 0.0f };
