...
// Mixed-precision GJ: Gauss-Jordan inversion in FP32
// All arithmetic in float; caller casts Omega→float before calling and result→double after
void inv_float( float *result, float *mat, unsigned int dim ) {
    ...
    for ( unsigned int m = 0; m < dim; m++ ) {
        for ( unsigned int n = 0; n < dim; n++ ) {
            temp[ m * dim + n ] = mat[ m * dim + n ];
            result[ m * dim + n ] = ( m == n ) ? 1.0f : 0.0f;
        }
    }

    for ( unsigned int i = 0; i < dim; i++ ) {
        if ( temp[ i * dim + i ] == 0.0f ) {
        ...
        for ( unsigned int j = 0; j < dim; j++ ) {
            if ( i != j ) {
                ratio = temp[ j * dim + i ] / temp[ i * dim + i ];
                for ( unsigned int k = 0; k < dim; k++ ) {
                    temp[ j * dim + k ]   -= ratio * temp[ i * dim + k ];
                    result[ j * dim + k ] -= ratio * result[ i * dim + k ];
                }
            }
        }
    }
