...
void build_diag_inv( CSRMatrix *csr, double *diag_inv, unsigned int dim ) {
    ...
    for ( unsigned int blk = 0; blk < n_blocks; blk++ ) {
        ...
        inv_blk[0] = (a[4]*a[8]-a[5]*a[7]) / det;
        inv_blk[1] = (a[2]*a[7]-a[1]*a[8]) / det;
        inv_blk[2] = (a[1]*a[5]-a[2]*a[4]) / det;
        inv_blk[3] = (a[5]*a[6]-a[3]*a[8]) / det;
        inv_blk[4] = (a[0]*a[8]-a[2]*a[6]) / det;
        inv_blk[5] = (a[2]*a[3]-a[0]*a[5]) / det;
        inv_blk[6] = (a[3]*a[7]-a[4]*a[6]) / det;
        inv_blk[7] = (a[1]*a[6]-a[0]*a[7]) / det;
        inv_blk[8] = (a[0]*a[4]-a[1]*a[3]) / det;
    }
}
