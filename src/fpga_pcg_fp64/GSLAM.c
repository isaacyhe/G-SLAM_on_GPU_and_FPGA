...
/* Include device_graph (device-resident edge compute + CSR assembly) */
...
/* spmv_parallel retained for toUseDiff=0 cost path if needed,
 * but is no longer called in the main PCG loop. */
...
int main( int argc, char *argv[] ) {
    ...
    /* -----------------------------------------------------------------------
     * One-time setup: build fixed CSR structure and allocate device buffers.
     * ----------------------------------------------------------------------- */
    DeviceGraph dg;
    dg.init(zlist, size_of_zlist, size_of_hat_xs, dim, device_queue);

    /* No CPU CSRMatrix needed: PCG runs fully on device. */
    ...
    for ( unsigned int n = 1; n <= numberOfRounds; n ++ ) {
         ...
         *   1. MotionEdge CSR scatter on CPU  (inside dg.update)
         *   2. Upload hat_xs to device        (inside dg.update)
         *   3. Fused ObsEdge SYCL kernel      (inside dg.update)
         *   4. Retrieve csr_vals, xi to host  (inside dg.update)
         * ------------------------------------------------------------------- */
        dg.update(hat_xs, us, delta, lambda, mns, device_queue);

        if ( !toUseDiff ) {
            /* Cost computation requires the full edge list with hat_e.
             * We still build edges on CPU for this case so that the cost
             * calculation is correct.  The CSR/xi from the device path is
             * used for the PCG solve regardless. */
            struct Edge *edges_cost = (Edge*) malloc( sizeof( struct Edge ) * 4194304 );
            struct LandmarkKeysZList *lkz_cost = (LandmarkKeysZList*) malloc( sizeof( struct LandmarkKeysZList ) * 4194304 );
            unsigned int size_of_edges_cost = 0, size_lkz_cost = 0;
            make_edges( hat_xs, size_of_hat_xs, zlist, size_of_zlist, edges_cost, &size_of_edges_cost, lkz_cost, &size_lkz_cost );
            free( lkz_cost );
            for ( unsigned int i = 0; i < size_of_hat_xs - 1; i++ )
                MotionEdge_create( &edges_cost[ size_of_edges_cost + i ], i, i+1, hat_xs, us, delta, lambda, mns );
            size_of_edges_cost += size_of_hat_xs - 1;

            for( unsigned int i = 0; i < size_of_edges_cost; i ++ ) {
                double transposeOFE[ 1 * 3 ] = { 0.0 };
                tra( transposeOFE, edges_cost[ i ].hat_e, 3, 1 );
                double dot_productOFtEOmega[ 1 * 3 ] = { 0.0 };
                dot( dot_productOFtEOmega, transposeOFE, 1, 3, edges_cost[ i ].Omega, 3, 3 );
                double dot_productOFtEOmegaE[ 1 * 1 ] = { 0.0 };
                dot( dot_productOFtEOmegaE, dot_productOFtEOmega, 1, 3, edges_cost[ i ].hat_e, 3, 1 );
                cost += dot_productOFtEOmegaE[ 0 ] * edges_cost[ i ].lambda;
            }
            free( edges_cost );
        }

        /* Full PCG on device — no CPU vectors needed */
        double *delta_xs = (double*)calloc(dim, sizeof(double));
        dg.pcg_solve(device_queue, 500, delta_xs);
    ...
    /* Release device graph resources */
    dg.free_all();

    /* Final map landmark estimation pass */
...
void make_edges( struct HAT_X *hat_xs, unsigned int size_of_hat_xs, struct Z *zlist, unsigned int size_of_zlist, struct Edge *edges, unsigned int *size_of_edges, struct LandmarkKeysZList *landmark_keys_zlist, unsigned int *size_of_landmark_keys_zlist ) {
	...
	*size_of_edges = sum;
	...
	unsigned int *lm_pos       = (unsigned int *)malloc( (max_id + 1) * sizeof(unsigned int) );
	unsigned int *lm_edge_base = (unsigned int *)malloc( (max_id + 1) * sizeof(unsigned int) );
	{
		unsigned int pos = 0, edge_base = 0;
		for ( unsigned int i = 0; i < max_id + 1; i++ ) {
			lm_pos[i]       = pos;
			lm_edge_base[i] = edge_base;
			if ( id_list[i] != max_id + 1 ) {
				pos       += id_count[ id_list[i] ];
				edge_base += (unsigned int)combination( id_count[ id_list[i] ], 2 );
			}
		}
	}
	...
	for ( unsigned int i = 0; i < max_id + 1; i ++ ) {
		if ( id_list[ i ] == max_id + 1 ) continue;
		unsigned int n    = id_count[ id_list[ i ] ];
		unsigned int pos  = lm_pos[ i ];
		unsigned int base = lm_edge_base[ i ];
		unsigned int local_count = 0;
		double snr[2] = { 0.14, 0.05 };
		for ( unsigned int j = 0; j < n; j ++ ) {
			for ( unsigned int k = j + 1; k < n; k ++ ) {
				ObsEdge_create( &edges[ base + local_count ],
				                landmark_keys_zlist[ pos + j ].z,
				                landmark_keys_zlist[ pos + k ].z,
				                hat_xs, snr );
				local_count ++;
			}
		}
