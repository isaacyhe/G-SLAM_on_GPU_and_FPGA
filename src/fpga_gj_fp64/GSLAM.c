...
int main( int argc, char *argv[] ) {
    ...
    for ( unsigned int n = 1; n <= numberOfRounds; n ++ ) {
        ...
        inv_parallel( inverse, Omega, dim );
...
void inv_parallel( double *result, double *mat, unsigned int dim ) {
    ...
    double *ratio = (double*)calloc( DIM, sizeof( double ) );
    double *temp = (double*)malloc( sizeof( double ) * DIM * DIM );
    memcpy( temp, mat, sizeof( double ) * DIM * DIM );

    buffer<double, 1> result_buf(result, dim_2);
    buffer<double, 1> ratio_buf(ratio, dim_1);
    buffer<double, 1> temp_buf(temp, dim_2);
    ...
    buffer<double, 1> result_buf(result, range<1>{dim * dim});
    buffer<double, 1> ratio_buf(ratio,   range<1>{dim});
    buffer<double, 1> temp_buf(temp,     range<1>{dim * dim});
