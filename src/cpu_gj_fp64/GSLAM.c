...
int main( int argc, char *argv[] ) {
    ...
    for ( unsigned int n = 1; n <= numberOfRounds; n ++ ) {
        ...
        if ( toUseDiff ) {
            ...
            /* Stop when even-round or odd-round diff hasn't improved for 10 each */
            if ( n % 2 == 0 ) {
...
void inv_parallel( double *result, double *mat, unsigned int dim ) {
    ...
    buffer<double, 1> result_buf(result, range<1>{(size_t)dim * dim});
    buffer<double, 1> ratio_buf(ratio,   range<1>{dim});
    buffer<double, 1> temp_buf(temp,     range<1>{(size_t)dim * dim});
