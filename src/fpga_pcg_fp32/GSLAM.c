...
/* spmv_parallel removed — SpMV is now done inside dg.pcg_solve() on device */
...
int main( int argc, char *argv[] ) {
    ...
    for ( unsigned int n = 1; n <= numberOfRounds; n ++ ) {
        ...
        if ( !toUseDiff ) {
             ...
             * Build edges on CPU for this case. */
        ...
        /* Device-resident PCG: build diag_inv + full solve on device */
