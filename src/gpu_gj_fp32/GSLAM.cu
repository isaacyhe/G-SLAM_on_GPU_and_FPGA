...
int main( int argc, char *argv[] ) {
    ...
    float best_diff_even = 1e30f, best_diff_odd = 1e30f;
    ...
    for ( unsigned int n = 1; n <= numberOfRounds; n ++ ) {
        ...
        float mns[4] = { 0.19f, 0.001f, 0.13f, 0.2f };
        ...
        *size_of_edges = *size_of_edges + *size_of_hat_xs - 1;
	    ...
	    for( unsigned int i = 0; i < *size_of_edges; i ++ ) {
                float transposeOFE[ 1 * 3 ] = { 0.0f };
                tra( transposeOFE, edges[ i ].hat_e, 3, 1 );

                float dot_productOFtEOmega[ 1 * 3 ] = { 0.0f };
                dot( dot_productOFtEOmega, transposeOFE, 1, 3, edges[ i ].Omega, 3, 3 );

                float dot_productOFtEOmegaE[ 1 * 1 ] = { 0.0f };
    	...
    	float *Omega = (float *)malloc ( sizeof( float ) * dim * dim ); // dim * dim

        for ( unsigned int j = 0; j < dim; j ++ ) {
            for ( unsigned int k = 0; k < dim; k ++ ) {
                if ( j == k && j < 3 ) {
                    Omega[ dim * j + k ] = 1000000.0f;
                }
        ...
        float *xi = (float *)malloc ( sizeof( float ) * dim ); // dim
        ...
        float *delta_xs = (float *)malloc ( sizeof( float ) * dim ); // dim
        float *inverse = (float *)malloc ( sizeof( float ) * dim * dim ); // dim * dim;
	...
	float *inverse_d, *xi_d, *delta_xs_d;
	cudaMalloc((void **)&inverse_d, sizeof(float) * dim * dim);
	//cudaMalloc((void **)&Omega_d, sizeof(float) * dim * dim);
	cudaMalloc((void **)&xi_d, sizeof(float) * dim);
	cudaMalloc((void **)&delta_xs_d, sizeof(float) * dim);

	//cudaMemcpy(Omega_d, Omega, sizeof(float) * dim * dim, cudaMemcpyHostToDevice);
	cudaMemcpy(xi_d, xi, sizeof(float) * dim, cudaMemcpyHostToDevice);

        inv_cuda( inverse_d, Omega, dim );
        
	//cudaMemcpy(inverse, inverse_d, sizeof(float) * dim * dim, cudaMemcpyDeviceToHost);
        ...
        dot_cuda(delta_xs_d, inverse_d, dim, dim, xi_d, dim, 1, dim);

	cudaMemcpy(delta_xs, delta_xs_d, sizeof(float) * dim, cudaMemcpyDeviceToHost);
	...
	/*for ( unsigned int i = 0; i < dim; i ++ ) {
        ...
        if ( toUseDiff )
            diff = vec_norm( delta_xs, dim, 1 ) / sqrtf( (float)dim );
        
	else
	    diff = fabsf( cost - last_cost ) / cost;
    ...
    unsigned int *count_of_landmark_ids = (unsigned int *)calloc( 1024, sizeof( unsigned int ) );
    ...
    for ( unsigned int i = 0; i < *size_of_landmark_keys_zlist; i ++ ) {
		...
		if ( i == *size_of_landmark_keys_zlist - 1 || ( i < *size_of_landmark_keys_zlist - 1 && landmark_keys_zlist[ i ].landmark_id != landmark_keys_zlist[ i + 1 ].landmark_id ) ) {
			...
			float *m = (float *)calloc( 2 * count_of_map_edges, sizeof( float ) );//128
...
void state_transition( float *result, float nu, float omega, unsigned int time, struct HAT_X pose ) {
    float t0 = pose.hat_x[2];

    if ( fabsf( omega ) < 1e-10f ) {
        float vector[3] = { nu * cosf( t0 ), nu * sinf( t0 ), omega };

        float multiplication[ 3 ] = { 0.0f };
        mul( multiplication, vector, 3, 1, time );

        float addition[ 3 ] = { 0.0f };
        ...
        float vector[3] = { nu / omega * sinf( t0 + omega * time ) - nu / omega * sinf( t0 ), nu / omega * cosf( t0 ) - nu / omega * cosf( t0 + omega * time ), omega * time };
...
unsigned int read_data( char *file_name, float *delta, struct U *us, unsigned int *size_of_us, struct HAT_X *hat_xs, unsigned int *size_of_hat_xs, struct Z *zlist, unsigned int *size_of_zlist  ) {
    char line[1024];
    FILE *input_file;
    ...
    while (fgets(line, sizeof(line), input_file) != 0) {
        ...
        if ( strcmp( token, "u" ) == 0 ) {
            ...
            token = strtok(NULL, " ");
            step_index = (unsigned int)atoi( token );
            us[ num_of_us ].step = step_index;

            token = strtok(NULL, " ");
            us[ num_of_us ].nu = (float)strtof( token, NULL );

            token = strtok(NULL, " ");
            us[ num_of_us ].omega = (float)strtof( token, NULL );
            ...
            token = strtok(NULL, " ");
            hat_xs[ num_of_xs ].hat_x[0] = (float)strtof( token, NULL );

            token = strtok(NULL, " ");
            hat_xs[ num_of_xs ].hat_x[1] = (float)strtof( token, NULL );

            token = strtok(NULL, " ");
            hat_xs[ num_of_xs ].hat_x[2] = (float)strtof( token, NULL );
            ...
            token = strtok(NULL, " ");
            zlist[ num_of_zs ].landmark_id = (unsigned int)atoi( token );

            token = strtok(NULL, " ");
            zlist[ num_of_zs ].z[0] = (float)strtof( token, NULL );

            token = strtok(NULL, " ");
            zlist[ num_of_zs ].z[1] = (float)strtof( token, NULL );
