...
int main( int argc, char *argv[] ) {
	...
	// Allocate GPU memory for the inverse matrix
	double *inverse_d;
	cudaMalloc((void **)&inverse_d, sizeof(double) * dim * dim);
	...
	/*cudaError_t err = cudaGetLastError();
        ...
        // Perform matrix inversion on GPU (see inv.cu for implementation)
        inv_cuda( inverse_d, Omega, dim );

	/*cudaError_t error = cudaGetLastError();
        if(error != cudaSuccess)
        {
                printf("CUDA ERROR inv: %s\n", cudaGetErrorString(error));
        }*/

	// Copy inverted matrix back to CPU for subsequent operations
	cudaMemcpy(inverse, inverse_d, sizeof(double) * dim *dim, cudaMemcpyDeviceToHost);
