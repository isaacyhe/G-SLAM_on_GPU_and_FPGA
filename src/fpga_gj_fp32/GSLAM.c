...
int main( int argc, char *argv[] ) {
    ...
    double delta = 0.0; //double *delta = calloc ( 1, sizeof( double ) );

    unsigned int size_of_us = 0; //unsigned int *size_of_us = calloc ( 1, sizeof( unsigned int ) );
    unsigned int size_of_hat_xs = 0; //unsigned int *size_of_hat_xs = calloc ( 1, sizeof( unsigned int ) );
    unsigned int size_of_zlist = 0; //unsigned int *size_of_zlist = calloc ( 1, sizeof( unsigned int ) );
    ...
    for ( unsigned int n = 1; n <= numberOfRounds; n ++ ) {
        ...
        unsigned int size_of_edges = 0; //unsigned int *size_of_edges = calloc ( 1, sizeof( unsigned int ) );
        unsigned int size_of_landmark_keys_zlist = 0; //unsigned int *size_of_landmark_keys_zlist = calloc ( 1, sizeof( unsigned int ) );
        ...
        float *inverse_f = (float*) malloc( sizeof( float ) * dim * dim ); // dim * dim;
        float *Omega_f = (float*) malloc( sizeof( float ) * dim * dim ); // dim * dim;

        for ( unsigned int j = 0; j < dim; j ++ ) {
            for ( unsigned int k = 0; k < dim; k ++ ) {
                Omega_f[ dim * j + k ] = (float)Omega[ dim * j + k ];
            }
        }
        ...
        inv_parallel( inverse_f, Omega_f, dim );
        //inv_dim( inverse, Omega, dim );
        
        for ( unsigned int j = 0; j < dim; j ++ ) {
            for ( unsigned int k = 0; k < dim; k ++ ) {
                inverse[ dim * j + k ] = (double)inverse_f[ dim * j + k ];
            }
        }
        ...
        if ( toUseDiff )
            diff = norm( delta_xs, dim, 1 ) / std::sqrt( (double)dim );

	    else
	        diff = std::fabs( cost - last_cost ) / cost;
    ...
    unsigned int landmark_id = size_of_landmark_keys_zlist; //unsigned int landmark_id = *size_of_landmark_keys_zlist;

    unsigned int order_of_landmark_ids[ 1024 ] = { 0 }; //unsigned int *order_of_landmark_ids = calloc( 1024, sizeof( unsigned int ) );
    ...
    for ( unsigned int i = 0; i < size_of_landmark_keys_zlist; i ++ ) {
		...
		if ( i == size_of_landmark_keys_zlist - 1 || ( i < size_of_landmark_keys_zlist - 1 && landmark_keys_zlist[ i ].landmark_id != landmark_keys_zlist[ i + 1 ].landmark_id ) ) {
			...
			double mean[ 2 ] = { 0.0 }; //double *mean = calloc( 2, sizeof( double ) );
...
void state_transition( double *result, double nu, double omega, unsigned int time, struct HAT_X pose ) {
    ...
    if ( std::fabs(omega) < 1e-10 ) {
        double vector[3] = { nu * std::cos( t0 ), nu * std::sin( t0 ), omega };
        ...
        double vector[3] = { nu / omega * std::sin( t0 + omega * time ) - nu / omega * std::sin( t0 ), nu / omega * std::cos( t0 ) - nu / omega * std::cos( t0 + omega * time ), omega * time };
...
void inv_parallel( float *result, float *mat, unsigned int dim ) {
#ifdef FPGA_HARDWARE
    #define DIM 2400
    #define WORKGROUP_SIZE 400
    #define SIMD 16

    range<1> dim_1{DIM};
    range<1> dim_2{DIM * DIM};

    range global{DIM, DIM};
    range local{WORKGROUP_SIZE, WORKGROUP_SIZE};

    range global_1{DIM};
    range local_1{WORKGROUP_SIZE};
    ...
    float *ratio = (float*)calloc( DIM, sizeof( float ) );
    float *temp = (float*)malloc( sizeof( float ) * DIM * DIM );
    memcpy( temp, mat, sizeof( float ) * DIM * DIM );

    buffer<float, 1> result_buf(result, dim_2);
    buffer<float, 1> ratio_buf(ratio, dim_1);
    buffer<float, 1> temp_buf(temp, dim_2);

    for (unsigned int i = 0; i < DIM; i ++) {
        device_queue.submit( [&] ( sycl::handler &inv_r ) {
            auto _temp = temp_buf.get_access<sycl::access::mode::read>(inv_r);
            auto _ratio = ratio_buf.get_access<sycl::access::mode::write>(inv_r);

            inv_r.parallel_for(nd_range{global_1, local_1}, [=](nd_item<1> index) [[intel::scheduler_target_fmax_mhz(400), intel::num_simd_work_items(SIMD), sycl::reqd_work_group_size(1, 1, WORKGROUP_SIZE)]] {
                auto j = index.get_global_id(0);
                if( i != j ) {
                    _ratio[j] = _temp[j * DIM + i] / _temp[i * DIM + i];
                }
            });
        }).wait();

        device_queue.submit( [&] ( sycl::handler &inv_core ) {
            accessor _ratio(ratio_buf, inv_core, read_only, ext::oneapi::accessor_property_list{sycl::ext::oneapi::no_alias});
            auto _result = result_buf.get_access<sycl::access::mode::read_write>(inv_core);
            auto _temp = temp_buf.get_access<sycl::access::mode::read_write>(inv_core);

            inv_core.parallel_for(nd_range{global, local}, [=](nd_item<2> index) [[intel::scheduler_target_fmax_mhz(400), intel::num_simd_work_items(SIMD), sycl::reqd_work_group_size(1, WORKGROUP_SIZE, WORKGROUP_SIZE)]] {
                auto j = index.get_global_id(0);
                auto k = index.get_global_id(1);
                if(i != j) {
                    _temp[j * DIM + k] = _temp[j * DIM + k] - _ratio[j] * _temp[i * DIM + k];
                    _result[j * DIM + k] = _result[j * DIM + k] - _ratio[j] * _result[i * DIM + k];
                }
            });
        }).wait();
    }

    device_queue.submit( [&] ( sycl::handler &inv_res ) {
        auto _result = result_buf.get_access<sycl::access::mode::read_write>(inv_res);
        auto _temp = temp_buf.get_access<sycl::access::mode::read>(inv_res);

        inv_res.parallel_for(nd_range{global, local}, [=](nd_item<2> index) [[intel::scheduler_target_fmax_mhz(400), intel::num_simd_work_items(SIMD), sycl::reqd_work_group_size(1, WORKGROUP_SIZE, WORKGROUP_SIZE)]] {
            auto j = index.get_global_id(0);
            auto k = index.get_global_id(1);
            _result[ j * DIM + k ] = _result[ j * DIM + k ] / _temp[ j * DIM + j ];
        });
    }).wait();
...
#else  // CPU/GPU path: use runtime dim

    // Work-group size: wg*wg must not exceed device max_work_group_size.
    size_t max_wg = device_queue.get_device()
                       .get_info<sycl::info::device::max_work_group_size>();
    size_t wg = 1;
    while ((wg * 2) * (wg * 2) <= max_wg && (wg * 2) <= (size_t)dim)
        wg *= 2;
    // Also ensure dim is divisible by wg (fall back to 1 if needed).
    while (dim % wg != 0 && wg > 1)
        wg /= 2;

    range<1> global_1{dim};
    range<1> local_1{wg};
    range<2> global{dim, dim};
    range<2> local{wg, wg};
    ...
    buffer<float, 1> result_buf(result, range<1>{dim * dim});
    buffer<float, 1> ratio_buf(ratio,   range<1>{dim});
    buffer<float, 1> temp_buf(temp,     range<1>{dim * dim});
    ratio_buf.set_final_data(nullptr);
    temp_buf.set_final_data(nullptr);

    for (unsigned int i = 0; i < dim; i++) {
        device_queue.submit([&](handler &inv_r) {
            auto _temp  = temp_buf.get_access<access::mode::read>(inv_r);
            auto _ratio = ratio_buf.get_access<access::mode::write>(inv_r);
            inv_r.parallel_for(nd_range{global_1, local_1},
                [=](nd_item<1> index) {
                    auto j = index.get_global_id(0);
                    if (i != j)
                        _ratio[j] = _temp[j * dim + i] / _temp[i * dim + i];
        ...
        device_queue.submit([&](handler &inv_core) {
            accessor _ratio(ratio_buf, inv_core, read_only, ext::oneapi::accessor_property_list{sycl::ext::oneapi::no_alias});
            auto _result = result_buf.get_access<access::mode::read_write>(inv_core);
            auto _temp   = temp_buf.get_access<access::mode::read_write>(inv_core);
            inv_core.parallel_for(nd_range{global, local},
                [=](nd_item<2> index) {
                    auto j = index.get_global_id(0);
                    auto k = index.get_global_id(1);
                    if (i != j) {
                        _temp[j * dim + k]   -= _ratio[j] * _temp[i * dim + k];
                        _result[j * dim + k] -= _ratio[j] * _result[i * dim + k];
                    }
                });
        }).wait();
    }

    device_queue.submit([&](handler &inv_res) {
        auto _result = result_buf.get_access<access::mode::read_write>(inv_res);
        auto _temp   = temp_buf.get_access<access::mode::read>(inv_res);
        inv_res.parallel_for(nd_range{global, local},
            [=](nd_item<2> index) {
                auto j = index.get_global_id(0);
                auto k = index.get_global_id(1);
                _result[j * dim + k] /= _temp[j * dim + j];
            });
    }).wait();
